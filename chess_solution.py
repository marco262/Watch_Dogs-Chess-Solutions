...
##        self.piece = a[2]
##
##    def CanCapture(self, black):
##        if self.piece == 'B':
####            print self.x, self.y
####            print black.x, black.y
##            if abs(black.x - self.x) == abs(black.y - self.y):
##                return True
##        elif self.piece == 'R':
##            if black.x == self.x or black.y == self.y:
##                return True
##        elif self.piece == 'N':
##            if ((abs(black.x - self.x) == 2 and
##                 abs(black.y - self.y) == 1) or
##                (abs(black.x - self.x) == 1 and
##                 abs(black.y - self.y) == 2)):
##                return True
##        elif self.piece == 'Q':
##            if (black.x == self.x or black.y == self.y or
##                (abs(black.x - self.x) == abs(black.y - self.y))):
##                return True
##        elif self.piece == 'K':
##            if abs(black.x - self.x) == 1 or abs(black.y - self.y) == 1:
##                return True
##        return False
##
##    def Capture(self, black):
##        self.x = black.x
##        self.y = black.y
##
##    def __str__(self):
##        return "{} ({},{})".format(self.piece, self.x, self.y)
##
##    def __repr__(self):
...
##        
##
##def MakePieces(board):
##    bad_pieces = []
##    good_pieces = []
##    for y,m in enumerate(board):
##        for x,n in enumerate(m):
##            if n == 'p':
##                bad_pieces.append(Piece([x,y,n]).ToList())
##            elif not n == ' ':
##                good_pieces.append(Piece([x,y,n]).ToList())
##    return good_pieces, bad_pieces
##
##def FindPath(good_pieces, bad_pieces, output_str=""):
##    if bad_pieces == []:
##        print output_str
##        return
##    for m,gp in enumerate(good_pieces):
##        white = Piece(gp)
##        for n,bp in enumerate(bad_pieces):
##            black = Piece(bp)
##            if white.CanCapture(black):
##                gp_copy = deepcopy(good_pieces)
##                bp_copy = deepcopy(bad_pieces)
##                next_succ = "{}{},{}>{},{} ".format(
##                    white.piece, white.x, white.y, black.x, black.y)
##                white.Capture(black)
##                gp_copy[m] = white.ToList()
##                del bp_copy[n]
##                FindPath(gp_copy, bp_copy, output_str+next_succ)
